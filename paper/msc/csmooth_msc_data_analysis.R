...
perform_pairwise_tests <- function(data) {
  ...
  test_data <- data %>%
    ...
    mutate(
      ...
      method = factor(method, levels = c("no smoothing", "gaussian", "constrained"))
    )
...
# save to file
ggsave("~/Box Sync/Aizenberg_Documents/Papers/csmooth/figures/msc_motor_activation_accuracy_by_smoothing_method.png", width = 10, height = 4, dpi = 300)
ggsave("~/Box Sync/Aizenberg_Documents/Papers/csmooth/figures/msc_motor_activation_accuracy_by_smoothing_method.pdf", width = 10, height = 4)
