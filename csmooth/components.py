...
def find_common_edges(edge_src, edge_dst, edge_distances, nodes, resampled_image_data, node_labels,
    ...
    if G.is_directed():
        logging.warning("Graph is directed when it should be undirected. Converting to undirected graph.")
        G = G.to_undirected()
    ...
    logging.debug(f"Sampling fraction for component: {_sampling_fraction:.4f}")

    # sample nodes from the graph
    max_dseg_label, second_max_dseg_label = node_labels[np.argsort(node_counts)[-2:]]
    logging.debug(f"Max dseg label: {max_dseg_label}, second max dseg label: {second_max_dseg_label}")
...
def find_outliers(edge_counter, outlier_threshold=5):
    ...
    logging.debug(f"Mean count: {mean_count:.2f}, Std count: {std_count:.2f}, Threshold for outliers: {threshold:.2f}")
    outliers = [edge for edge, count in edge_counter.items() if count > threshold]
    logging.debug(f"Found {len(outliers)} outlier edges with counts above the threshold of {threshold:.2f}")
    return outliers
...
def check_components(edge_src, edge_dst, edge_distances, labels, unique_nodes, sorted_labels,
    ...
    """
    logging.info('Checking components')
    ...
    for label in sorted_labels[:n_components]:
        ...
        if np.max(node_counts_normalized) > qc_threshold:
            ...
            logging.debug(f"Number of components before removing edges: {n_components}")
            edges_removed = list()
            while n_components < 2:
                ...
                for edge_to_remove in outliers:
                    logging.debug(f"Removing edge {edge_to_remove} with count")
                    ...
                    logging.debug(f"Component edges before removing edge {edge_to_remove}: {len(_edge_src)} edges")
                    _edge_src = _edge_src[_edge_mask]
                    _edge_dst = _edge_dst[_edge_mask]
                    _edge_distances = _edge_distances[_edge_mask]
                    logging.debug(f"Component edges after removing edge {edge_to_remove}: {len(_edge_src)} edges remaining")
                    ...
                    logging.debug(f"Total number of edges before removing edge {edge_to_remove}: {len(edge_src)} edges")
                    ...
                    logging.debug(f"Total number of edges after removing edge {edge_to_remove}: {len(edge_src)} edges remaining")

                    edges_removed.append((edge_to_remove, removed_edge_distance))

                logging.debug(f"Checking connected components after removing edges")
                component_labels, sorted_component_labels, component_nodes = identify_connected_components(
                    edge_src=_edge_src,
                    edge_dst=_edge_dst,
                    edge_distances=_edge_distances)
                n_components = len(sorted_component_labels)
                logging.debug(f"Number of components after removing edges: {n_components}")
    ...
    end_time = time.time()
    logging.info(f"Time taken to check components: {(end_time - start_time) / 60:.2f} minutes")
